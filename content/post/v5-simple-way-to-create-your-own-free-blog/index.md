...
````md
diff --git a//home/fahad/hugo-theme-stack-starter/new-post/post-template-all-features.md b//home/fahad/hugo-theme-stack-starter/new-post/post-template-all-features.md
--- a//home/fahad/hugo-theme-stack-starter/new-post/post-template-all-features.md
+++ b//home/fahad/hugo-theme-stack-starter/new-post/post-template-all-features.md
@@ -1,7 +1,6 @@
-# قالب مقال شامل (All‑in‑one) — مناسب للمبتدئ جدًا (Hugo Stack)
+# قالب مقال شامل لكن بسيط — مناسب للمبتدئ جدًا (Hugo Stack)
 
-> هذا القالب مصمم لنظامك في `blog-post.md`:
-> - Page Bundle: `content/post/<slug>/index.md`
-> - ملفات الغلاف داخل نفس المجلد: `cover.png` + `cover-post-v1.png`
-> - أقسام المقال **بنفس الترتيب** (مقدمة → فيديو → متطلبات → خطوات → ليش → مشاكل → FAQ → خلاصة)
...
-description: "وصف مختصر وواضح: وش بتسوي بالضبط؟ ولمين؟ وبأي أداة؟"
...
-اكتبها بصيغة بسيطة جدًا:
+اكتبها بـ 3 أسطر:
 
-- **المشكلة**: وش اللي يضايقك/يوقفك؟
-- **الهدف**: وش تبي تسوي بالنهاية؟
-- **النتيجة**: وش بيطلع مع القارئ بعد المقال؟
+- وش المشكلة؟
+- وش الهدف؟
+- وش النتيجة اللي بيطلع فيها القارئ؟
 
-مثال صغير:
-> تبغى تسوي X بدون Y… وبأسهل طريقة للمبتدئ؟  
-> في هذا الدليل بنسوي Z خطوة بخطوة، وبنهاية المقال بيكون عندك: (…).
+مثال (قالب):
+> تبغى تسوي **[X]** بدون **[Y]**؟  
+> هنا بنسويها خطوة بخطوة، وبالنهاية بيكون عندك: **[Z]**.
 
-روابط داخلية (مثال):
-- [مقال مرتبط 1](/post/slug-1/)
-- [مقال مرتبط 2](/post/slug-2/)
-
-روابط خارجية (مثال):
-- [الموقع الرسمي للأداة](https://example.com)
+روابط (اختياري):
...
-اكتبها كبنود واضحة:
-
-- حساب/شيء لازم يكون جاهز (مثال: GitHub).
-- صلاحيات مطلوبة (مثال: دخول للراوتر).
-- جهاز/نظام (مثال: Raspberry Pi / Windows / Linux).
-
-مثال:
-- حساب GitHub.
-- متصفح.
-- (اختياري) صورة غلاف `cover.png`.
+3–6 نقاط تكفي:
+- …
+- …
+- …
...
-> قاعدة ذهبية للمبتدئ: **كل خطوة فيها: (وش تسوي؟ ليش؟ كيف تتأكد؟)**
...
-**كيف أتأكد؟**
-- اكتب “علامة نجاح” واضحة.
+**كيف أتأكد؟** اكتب علامة نجاح واضحة (مثال: “طلع ✅ في Actions”).
 
-> تلميح: تقدر تحط ملاحظة قصيرة كـ Blockquote بدون ما تطوّل.
+> ملاحظة قصيرة (اختياري): اكتب “ليش” بسطر واحد.
 
@@ -94,6 +80,3 @@
 
-### 4.3) خطوة 3 — صورة/سكرينشوت (Image)
-
-> ضع الصور داخل نفس مجلد المقال وبأسماء واضحة:
-> `step-01-....png`, `step-02-....png`
+### 4.3) خطوة 3 — صورة (اختياري)
 
@@ -101,3 +84,3 @@
 
-### 4.4) خطوة 4 — قائمة فحص (Checklist)
+### 4.4) خطوة 4 — قائمة فحص (اختياري)
 
@@ -107,3 +90,6 @@
 
-### 4.5) خطوة 5 — جدول (Table)
+<details>
...
-### 4.6) خطوة 6 — اقتباس (Quote) + مصطلحات
+### اقتباس
...
-
-مصطلحات (مثال):
-- `baseurl`: رابط موقعك النهائي.
-- `slug`: رابط المقال.
 
-### 4.7) خطوة 7 — تفاصيل اختيارية (Dropdown للمبتدئ)
+### Dropdown (معلومة متقدمة بدون تخويف)
 
@@ -137,3 +119,3 @@
 
-### 4.8) خطوة 8 — تنبيه “لا تسوي كذا”
+### تنبيه (لا تسوي كذا)
...
-هنا “نقاط قصيرة” تشرح ليش اخترت كذا:
+## 5) ليش الخيارات تفرق؟ (Why choices matter)
 
-- ليش استخدمنا (أداة/خيار) بدل (بديل).
-- وش بيخرب إذا غيرته.
-- متى ممكن تغيره لاحقًا.
+3 نقاط قصيرة تكفي:
+- ليش اخترنا كذا؟
+- وش بيخرب لو غيرته؟
+- متى تغيّره؟
