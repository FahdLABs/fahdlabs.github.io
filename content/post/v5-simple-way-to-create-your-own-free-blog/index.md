---
title: "دليل المبتدئين جدًا: سوّ مدونة عربية بـ Hugo Stack خطوة بخطوة (GitHub Pages)"
description: "شرح عملي للمبتدئ: انسخ القالب لحسابك، فعّل GitHub Pages، عدّل من المتصفح، اضبط baseurl، عربه RTL، اكتب أول مقال، وانشر."
slug: "hugo-stack-arabic-beginners-guide"
...
8. احذف أو اخفِ المقالات الافتراضية
...
الخطوات:
1. افتح ريبو القالب على GitHub (وإذا عجبك اعطه نجمة).
...
> **أول مرة البلد ممكن تفشل** — هذا طبيعي لأن GitHub Actions ما كان مفعّل بعد.
> بعد ما تفعّله (خطوة 4.2)، ارجع لـ `Actions` > اضغط على `build and deploy` > اضغط `Run workflow` عشان يحاول يبني الموقع مرة ثانية.
...
> **تبغى تخفي صفحة من القائمة؟** (مثلاً الأرشيف)
> افتح `config/_default/menu.toml` واعمل Comment لسطور الصفحة اللي تبغى تخفيها — في GitHub Web UI اضغط `Ctrl + /` على الأسطر المحددة.
...
> **تنبيه:** القالب يجي معه أيقونات محدودة جاهزة (مثل GitHub و Twitter).
> أيقونات مثل **YouTube** و **Instagram** مو موجودة بشكل افتراضي — لازم تسوي ملف SVG لكل وحدة وتحطه في `assets/icons/`.
> شوف قسم **"إضافة أيقونات SVG بنفسك"** في آخر المقال لشرح الطريقة بالتفصيل.
...
**طريقة 1: إخفاء (draft)**
1. افتح ملف المقال (مثلاً `content/post/hello-world/index.md`).
2. اضغط أيقونة القلم `Edit`.
3. أضف هذا السطر في بطاقة المقال (بين الـ `---`):
...
> `draft: true` يخفي المقال من الموقع لكنه يبقى في الريبو — تقدر ترجعه لاحقًا.

**طريقة 2: حذف نهائي**
1. افتح مجلد المقال داخل `content/post/`.
2. اضغط على الملف اللي تبغى تحذفه.
3. اضغط النقاط `...` (أعلى يمين) > `Delete file`.
4. اضغط `Commit changes`.
...
1. افتح مجلد `content/post/` في الريبو.
2. اضغط `Add file` > `Create new file`.
3. في خانة الاسم اكتب: `my-first-post/index.md`
   - أول جزء قبل الـ `/` يصير اسم المجلد.
   - `index.md` هو ملف المقال (لازم يكون بهذا الاسم بالضبط).
...
1. ادخل مجلد المقال (`content/post/my-first-post/`).
2. اضغط `Add file` > `Upload files`.
3. ارفع صورة باسم `cover.png` (أو `cover.jpg`).
4. اضغط `Commit changes`.
5. ارجع لملف `index.md` وأضف في البطاقة:
...
مهم:
- **لا تكتب `image: cover.png` إلا إذا الصورة موجودة فعلاً** — وإلا الموقع يعطيك خطأ.
- نفس الشيء لأي صورة داخل المقال: لازم الملف يكون موجود في نفس المجلد بنفس الاسم.
...
5. انتظر لين يجهّز البيئة (أول مرة ياخذ دقيقة أو أكثر).
...
- بيطلع لك رابط محلي (مثل `http://localhost:1313/`).
- Codespaces بيعرض لك إشعار — اضغط `Open in Browser`.
- الآن كل ما تحفظ ملف، الصفحة تتحدث لحظيًا.

### عدّل وجرّب

- عدّل أي ملف (مقال، إعداد، ستايل…).
- احفظ (`Ctrl + S`).
- شوف النتيجة فورًا في المتصفح.
...
> **ملاحظة:** لا تنسى توقف الـ Codespace لما تخلص عشان ما تستهلك الساعات المجانية.
> ادخل [github.com/codespaces](https://github.com/codespaces) وأوقف أي Codespace شغال.
...
> **تنبيه مهم جدًا:**
> كل صورة تذكرها في المقال **لازم تكون موجودة فعليًا داخل نفس مجلد المقال**.
> مثلاً إذا كتبت `![وصف](screenshot1.png)` لازم يكون ملف `screenshot1.png` موجود جنب `index.md`.
> **إذا الصورة مو موجودة، الموقع بيعطيك خطأ وما بيشتغل!**

استخدمه كـ Page Bundle:
- أنشئ مجلد: `content/post/<slug>/`
- داخله ملف: `index.md`
- داخل **نفس المجلد**: حط صورة الغلاف `cover.png` وأي صور ثانية تبغى تستخدمها بالمقال

طريقة إضافة صورة داخل المقال:
> `![وصف واضح للصورة](اسم-الصورة.png)`
...
![وصف واضح للصورة: وش اللي نراه هنا؟](screenshot1.png)
